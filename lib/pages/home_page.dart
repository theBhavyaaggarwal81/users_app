...
class _HomePageState extends State<HomePage>
{
  ...
  retrieveDirectionDetails() async
  {
    ...
    showDialog(
      ...
      context: context,
      builder: (BuildContext context) => LoadingDialog(messageText: "Fetching Route..."),
    );
  ...
  updateAvailableNearbyOnlineDriversOnMap(){
    ...
    for(OnlineNearbyDrivers eachOnlineNearbyDriver in ManageDriversMethods.nearbyOnlineDriversList){
      LatLng driverCurrentPostiion = LatLng(eachOnlineNearbyDriver.latDriver!, eachOnlineNearbyDriver.lngDriver!);

      Marker driverMaker = Marker(
        markerId: MarkerId("driver ID = "+ eachOnlineNearbyDriver.uidDriver.toString()),
        position: driverCurrentPostiion,
        icon: carIconNearbyDriver!,
      );

      markersTempSet.add(driverMaker);
    }
      ...
      if(driverEvent!=null)
        {
          ...
          switch(onlineDriverChild){
            ...
            case Geofire.onKeyMoved:
              ...
              onlineNearbyDrivers.latDriver = driverEvent["latitude"];
              onlineNearbyDrivers.lngDriver = driverEvent["longitude"];
              ManageDriversMethods.updateOnlineNearbyDriversLocation(onlineDriverChild);
  ...
  searchDriver(){
    if(availableNearbyOnlineDriversList!.length == 0)
      ...
      //send push notification to current  driver - selected driver
  ...
  sendNotificationToDriver(OnlineNearbyDrivers currentDriver){
    //update driver's newTripStatus - assign tripID to currentDriver
    ...
    //get current driver's registration token
    ...
    tokenOfCurrentDriverRef.once().then((dataSnapshot){
      ...
      var timerCountDown = Timer.periodic(oneTickPerSec, (timer) {
        ...
        //when trip request is not requesting means trip request cancelled
        ...
        //if 20 seconds passed - send notification to the next online available driver
        if(requestTimeoutDriver == 0){
          ...
          //send notification to the next online available driver
          searchDriver();
        }
      });
    });
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        ClipOval(
                          child: Image.network(
                            photoDriver == ''
                                ? "https://firebasestorage.googleapis.com/v0/b/uber-flutter-clone-usersapp.appspot.com/o/avatarman.png?alt=media&token=1bebb34e-1ae2-4f7d-a613-ec6bca93083c"
                                : photoDriver,
                            width: 60,
                            height: 60,
                            fit: BoxFit.cover,
